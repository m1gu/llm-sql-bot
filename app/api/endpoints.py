...
ENDPOINT_SPECS: Dict[str, EndpointSpec] = {
    ...
    "analytics_customers_orders_summary": EndpointSpec(
        name="analytics_customers_orders_summary",
        method="GET",
        path="/analytics/customers/orders/summary",
        description="Resumen compacto de órdenes por cliente, resolviendo nombres parciales o IDs.",
        params=[
            EndpointParam("customer_id", "Identificador directo del cliente"),
            EndpointParam("customer_name", "Nombre libre/parcial del cliente"),
            EndpointParam("match_strategy", "best o all para controlar coincidencias"),
            EndpointParam("match_threshold", "Puntaje mínimo de coincidencia (0-1)"),
            EndpointParam("date_from", "Fecha inicial del rango evaluado"),
            EndpointParam("date_to", "Fecha final del rango evaluado"),
            EndpointParam("sla_hours", "SLA aplicado si se requiere override"),
            EndpointParam("include_samples", "Incluye agregados de samples pendientes"),
            EndpointParam("include_tests", "Incluye agregados de tests pendientes"),
            EndpointParam("limit_orders", "Máximo de órdenes detalladas"),
        ],
    ),
    "entities_order_detail": EndpointSpec(
        name="entities_order_detail",
        method="GET",
        path="/entities/orders/{order_id}",
        description="Detalle enriquecido de una orden con SLA y entidades vinculadas.",
        params=[
            EndpointParam("order_id", "ID de la orden (path)", True),
            EndpointParam("sla_hours", "SLA en horas (default 48)"),
            EndpointParam("include_samples", "Adjuntar samples relacionados"),
            EndpointParam("include_tests", "Adjuntar tests (implica include_samples)"),
        ],
    ),
    "entities_sample_full": EndpointSpec(
        name="entities_sample_full",
        method="GET",
        path="/entities/samples/{sample_id}/full",
        description="Detalle completo de un sample, con orden/tests/batches y SLA.",
        params=[
            EndpointParam("sample_id", "ID del sample (path)", True),
            EndpointParam("sla_hours", "SLA en horas (default 48)"),
            EndpointParam("include_tests", "Adjuntar tests relacionados"),
            EndpointParam("include_batches", "Adjuntar batches relacionados"),
        ],
    ),
    "entities_test_full": EndpointSpec(
        name="entities_test_full",
        method="GET",
        path="/entities/tests/{test_id}/full",
        description="Detalle completo de un test con referencias a sample/orden.",
        params=[
            EndpointParam("test_id", "ID del test (path)", True),
            EndpointParam("sla_hours", "SLA en horas (default 48)"),
            EndpointParam("include_sample", "Adjuntar sample relacionado"),
            EndpointParam("include_order", "Adjuntar orden relacionada"),
            EndpointParam("include_batches", "Adjuntar batches"),
            EndpointParam("include_raw_worksheet", "Incluir worksheet bruto"),
        ],
    ),
